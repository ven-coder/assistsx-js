...
export class AXWebDev {
    ...
    // 统一的调用方法
    private static async call<T>(method: string, args?: string, node?: Node): Promise<T | null> {
        try {
            const params = {
                method,
    ...
    // 设置节点文本
    public static async setNodeText(nodeId: string, text: string): Promise<boolean> {
